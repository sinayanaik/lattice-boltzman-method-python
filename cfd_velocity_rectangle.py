...
def main():
    Nx = 450 # no of x dimension points
    Ny = 350 # no of y dimension points
    tau = .9 # timescale corresponds to kinematic viscosity
    ...
    for y in range(0,Ny):
        for x in range(0,Nx):
            if x > Nx/4 - side_length and x < Nx/4 + side_length and y > Ny/2 - side_length - 25 and y < Ny/2 + side_length + 25:
                space[y][x] = True
    ...
    for iteration in range(0,iter_count):
        ...
        if(iteration % plot_every == 0):
            ...
            pyplot.title("Fluid flowing through the Rectangular obstacle")
