...
def main():
    ...
    # rectangular obstacle with sides of 30 cm x 30 cm in the middle
    for y in range(0,Ny):
        for x in range(0,Nx):
            if x > Nx/2 - 15 and x < Nx/2 + 15 and y > Ny/2 - 15 and y < Ny/2 + 15:
                obstacle[y][x] = True
    ...
    for iteration in range(0,Nt):
        ...
        if(iteration % plot_every == 0):
            pyplot.imshow(np.sqrt(ux**2 + uy**2),cmap="tab20b") # Set3 , tab20b , PuBu
            pyplot.pause(0.1)
            pyplot.cla()
